...
export const getUnapprovedVendorList = async (req, res) => {
    try {
        const unapprovedVendorList = await User.find({ role: 'vendor', isApprovedByAdminIfVendorRole: false });
...
export const approveVendorProfile = async (req, res) => {
    const { id: _id } = req.params;
    try {
        if (!mongoose.Types.ObjectId.isValid(_id)) return res.status(404).send('No user with that id');
        const updatedVendorProfile = await User.findByIdAndUpdate(_id, { isApprovedByAdminIfVendorRole: true, _id }, { new: true });
        const mailOptions = {
...
export const getVendorDeletionRequestList = async (req, res) => {
    try {
        const deletionRequestList = await User.find({ role: 'vendor', isProfileDeletionRequested: true });
...
export const requestVendorDeletion = async (req, res) => {
    ...
    try {
        if (!mongoose.Types.ObjectId.isValid(_id)) return res.status(404).send('No user with that id');
        const updatedVendorProfile = await User.findByIdAndUpdate(_id, { isProfileDeletionRequested: true, _id }, { new: true });
...
export const declineVendorDeletion = async (req, res) => {
    const { id: _id } = req.params;
    try {
        if (!mongoose.Types.ObjectId.isValid(_id)) return res.status(404).send('No user with that id');
        const updatedVendorProfile = await User.findByIdAndUpdate(_id, { isProfileDeletionRequested: false, _id }, { new: true });
...
export const declineVendorCreation = async (req, res) => {
    const { id: _id } = req.params;
    ...
    try {
        ...
        transporter.sendMail(mailOptions, function (error, info) {
            if (error) {
                ...
                res.json({ message: 'Vendor profile creation request declined successfully.' });
            }
        });
    } catch (error) {
        res.status(500).json({ message: "Something went wrong." });
